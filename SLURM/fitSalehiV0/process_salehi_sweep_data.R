...
fitDir <- "data/salehi/alfak_outputs/"
...
prediction_res <- function(dj,fi,mi){
  ...
  df_de <- data.frame(pred=sqrt(sum((kp-kt)^2)),
  ...
  sb <- wpp(k,mass=yb)
  sp <- wpp(k,mass=yp)
  df_dw <- data.frame(pred=wasserstein(st,sp),
  ...
  df_sc <- data.frame(pred=sc_p,base=sc_0,metric="cosine")
  
  ##overlap:
  so_0 <- sum(pmin(yb, yt))/min(sum(yb), sum(yt))
  so_p <- sum(pmin(yp, yt))/min(sum(yp), sum(yt))
  df_so <- data.frame(pred=so_p,base=so_0,metric="overlap")
...
df <- pbapply::pblapply(ff,function(fi){
  print(fi)
  ...
  dfi <- do.call(rbind,lapply(min_obs,function(mi){
    ...
    xp0 <- data.frame(pred=NaN,base=NaN,metric=NaN,test_treat=NaN)
    
    xp <- lapply(li$dec1,prediction_res,fi=fi,mi=mi)
    xp <- xp[!sapply(xp,is.null)]
    xp <- do.call(rbind,xp)
    if(is.null(xp)) return(cbind(xv,xp0))
    return(cbind(xv,xp))
  }))
})
...
df <- do.call(rbind,df)
saveRDS(df,"data/salehi/alfak_outputs_proc.Rds")

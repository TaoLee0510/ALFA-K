...
p_bar <- ggplot(z_bar, aes(type, fwin, fill = n)) +
  facet_grid(rows = vars(metric)) +
  geom_col() +
  coord_flip() +
...
# --- Plot: combined ECDF ---
p_ecdf <- ggplot() +
  stat_ecdf(data=z_euc,      aes(angle),     geom="step", size=1, color="blue")   +
  stat_ecdf(data=data.frame(angle=subset(y, metric=="angle")$value),
            aes(angle),     geom="step", size=1, color="darkgreen") +
  stat_ecdf(data=z_pairs,     aes(angle),     geom="step", size=1, color="purple") +
  geom_line(data=nulldf,      aes(angle, CDF), color="red", size=1, linetype="dashed") +
  labs(x="Angle (degrees)", y="Cumulative Probability") +
  base_theme
...
p_ref_null <- ggplot(plot_df_ref, aes(lineage, win, fill=n)) +
  facet_grid(rows=vars(metric)) +
  geom_col() +
  geom_label(aes(label=paste0("n=",n), hjust=0),
             size=base_text_size*0.375, fill="white", color="black", label.size=0.2) +
  coord_flip() +
  scale_x_discrete("") +
...
##############################
# Section 2: Network / Dendrogram Plot (from Script 1)
##############################

# --- Utility to prune metadata tree ---
...
df_meta <- readRDS("figures/misc/data/annotated_metadata.Rds") %>%
  prune_children()
sample_lineage_map2 <- c(
  SA1035 = "SA1035", SA532 = "SA532", SA609 = "SA609",
  SA906  = "p53 k.o", SA039 = "p53 w.t", SA535 = "SA535"
)
df_meta$linlab <- sample_lineage_map2[df_meta$PDX_id]

dummy_uid <- "ROOT"
dummy_row <- df_meta[1, ]; dummy_row$uid <- dummy_uid; dummy_row$datasetname <- dummy_uid
df_fixed <- bind_rows(df_meta, dummy_row)
df_fixed$parent[is.na(df_fixed$parent) | df_fixed$parent==""] <- dummy_uid
df_fixed$lineage <- ifelse(df_fixed$datasetname %in% names(sample_lineage_map2),
                           sample_lineage_map2[df_fixed$datasetname], "TNBC PDX")
df_fixed$lineage[grepl("x", df_fixed$uid)] <- "dummy"
df_fixed$lineage[df_fixed$uid==dummy_uid] <- "dummy"

edges_df <- df_fixed %>%
  filter(uid != dummy_uid) %>%
  transmute(from=parent, to=uid, treatment=on_treatment,
            from_dummy=(parent==dummy_uid))
g <- graph_from_data_frame(edges_df, vertices=df_fixed, directed=TRUE)

tg <- as_tbl_graph(g) %>%
  mutate(dummy=(lineage=="dummy"),
         node_type=ifelse(dummy,"dummy",
                          ifelse(parent==dummy_uid,"root","non-root")))
...
radius_multipliers <- c(
  "p53 w.t"=5, "SA1035"=6, "SA532"=10,
  "SA535"=8,   "SA609"=5, "p53 k.o"=8
)
lineage_labels <- layout %>%
  filter(parent==dummy_uid) %>%
  mutate(r=sqrt(x^2+y^2), theta=atan2(y,x),
         r_label=r*radius_multipliers[linlab],
...
p_network <- ggraph(layout) +
  ...
  geom_edge_link(aes(color=ifelse(from_dummy,"dummy",treatment)),
                 edge_width=1) +
  geom_node_point(data=filter(layout,dummy), aes(x,y), color="white") +
  geom_node_point(data=filter(layout,!dummy), aes(x,y,color=xval), size=4) +
  scale_edge_color_manual(name="", values=c("y"="red","n"="black","dummy"="white"),
                          labels=c("y"="Treatment ON","n"="Treatment OFF","dummy"="")) +
  scale_color_viridis_c("CV\nscore") +
  geom_text(data=lineage_labels,
            aes(x=x_label,y=y_label,label=linlab), fontface="bold") +
  text_size_theme +
...
# Example: print(p_ecdf)
# print(plots$salehi_ecdf)

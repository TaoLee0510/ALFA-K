...
## similar to cna_mat, consider merging
assess_nkary <- function(fi){
  uid <- x$uid[x$filenames==fi]
  pdx <- m$PDX_id[m$uid==uid]
  lab <- m$label[m$uid==uid]
  xi <- readRDS(paste0("data/salehi/alfak_fits_post_jump/minobs_5/",fi))
  nkary <- sum(xi$xo$id=="fq")
  data.frame(uid,pdx,lab,nkary)
}

x <- xx[xx$min_obs==5&!xx$has_descendents&!xx$has_parents&xx$r2>0,]
ff <- x$filenames

df <- do.call(rbind,lapply(ff,assess_nkary))
df$treatment_status <- sapply(df$uid, function(id){
  x$treatment_status[x$uid==id]
})
...
df_agg <- aggregate(list(sd=df$nkary),by=list(type=df$type,treatment=df$treatment_status),sd,na.rm=T)
df_agg$mean <- aggregate(list(mean=df$nkary),by=list(type=df$type,treatment=df$treatment_status),mean,na.rm=T)$mean
df_agg$labs <- c("in-vitro","PDX","PDX+C")
...
pc_old <- ggplot(df[df$type=="pdx",],aes(x=deltaf))+
...
```{r,echo=FALSE,include=FALSE,eval=FALSE}
meta <- readRDS("figures/salehi_data_fitting/labelled_metadata.Rds")
x <- xx[grepl("SA1035",xx$filenames)&xx$r2>0&xx$min_obs==5,]
...
ggsave("figures/salehi_landscape_similarity/figs/figure.png",plot = plot,width=9,height=9,units="in",bg="white")
```

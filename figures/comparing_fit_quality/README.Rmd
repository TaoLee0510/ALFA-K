...
Before diving into the analysis, it is useful to know that there are several "frequent clones" shared between hTERTa and hTERTb, and there is a decent amount of correlation between our fitness estimates on them.

NEW

```{r,echo=FALSE}
dir <- "data/salehi/alfak_fits_minobs_adaptive/"
hTERTa <- get_best(readRDS(paste0(dir,"p53--a_X57.Rds")))$xv_res
hTERTb <- get_best(readRDS(paste0(dir,"p53--b_X55.Rds")))$xv_res
...
dir <- "data/salehi/"
xcomb <-  get_best(readRDS(paste0(dir,"alfak_fits_htert_combined.Rds")))$xv_res
#xcomb$loo_pred <- 0.2+xcomb$loo_pred-min(xcomb$loo_pred)
#xcomb$f_est <- 0.2+xcomb$f_est-min(xcomb$f_est)

xsep$id2 <- "separate"
xcomb$id2 <- "combined"
colnames(xsep)[colnames(xsep)=="f_xv"] <- "loo_pred"
x <- rbind(xsep,xcomb)

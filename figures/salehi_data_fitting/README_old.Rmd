...
Characterize the ploidy of each of the cell lines:
...
x <- do.call(rbind,lapply(m, function(msel){
  xi <- cnmat[msel$library_id]
  xi <- do.call(rbind,xi)
  xi <- as.numeric(rowMeans(xi))
  data.frame(id=msel$PDX_id[1],ploidy=xi)
}))

p <- ggplot(x,aes(x=ploidy))+
  facet_wrap(~id)+
  geom_histogram()
p
```
A more relevant way of characterizing ploidy - look and say what the variability of the frequent clones is in the landscapes we have actually fitted:
...
x <- do.call(rbind,lapply(ff,function(fi){
  ...
  k <- rownames(xi$xv_res)
  ploidy <- rowMeans(do.call(rbind,lapply(k,s2v)))
  data.frame(id=fi,k,ploidy=ploidy)
}))
...
ff <- list.files("data/salehi/alfak_fits/minobs_10/")
ids <- sapply(ff,function(fi) head(unlist(strsplit(fi,split=".Rds")),1))
cellLines <- do.call(rbind,lapply(l[ids],lineage_info))$cellLine

x <- do.call(rbind,lapply(1:length(ff), function(i){
  xi <- readRDS(paste0("data/salehi/alfak_inputs_v2/",ff[i]))
  data.frame(k=rownames(xi$x),id=cellLines[i])
}))
...
x <- do.call(rbind,lapply(x,function(xi){
  xi <- unique(xi)
  id <- xi$id[1]
  ploidy <- sapply(xi$k, function(xi) mean(s2v(xi)))
  data.frame(ploidy,id)
}))
...
lineage_info <- function(ln){
  has_descendents <- length(ln$dec1)>0
  has_parents <- sum(is.na(m$parent[m$uid%in%ln$ids]))==0
  treatment_change <- length(unique(m$on_treatment[m$uid%in%ln$ids]))>1
  id <- tail(ln$ids,1)
  sid <- paste(gsub(" ","", m$label[id==m$uid],fixed = TRUE),
               m$timepoint[id==m$uid],sep = "_")
  data.frame(id=sid,has_descendents,has_parents,treatment_change)
...
x <- lapply(min_obs, function(mo){
  ...
  #ff <- ff[ff%in%linfo$filenames[!linfo$treatment_change]]
